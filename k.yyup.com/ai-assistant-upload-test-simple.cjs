...
async function testAIAssistantUpload() {
    ...
    try {
        ...
        const pageContent = await page.evaluate(() => {
            ...
            const buttons = Array.from(document.querySelectorAll('button, .btn, [role="button"]'))
                .map(btn => ({
                    ...
                    innerHTML: btn.innerHTML.substring(0, 200) // 限制长度
                }));
            ...
            const uploadElements = Array.from(document.querySelectorAll('*'))
                .filter(el => {
                    ...
                    return text.includes('上传') ||
                           ...
                           el.title?.toLowerCase().includes('上传');
                })
                .map(el => ({
                    tagName: el.tagName,
            ...
            const iconElements = Array.from(document.querySelectorAll('.icon, [class*="icon"], svg'))
                .map(icon => ({
                    ...
                    innerHTML: icon.outerHTML?.substring(0, 200)
                }));
        ...
        for (const btn of uploadButtons) {
            ...
            try {
                // 尝试找到并点击这个按钮
                const buttonSelector = `button:has-text("${btn.text}"), [title="${btn.title}"], .${btn.className.split(' ').join('.')}`;

                await page.click(buttonSelector, { timeout: 2000 });
                uploadButtonClicked = true;
                console.log(`✅ 成功点击上传按钮: ${btn.text || btn.title}`);
        ...
        const testReport = {
            ...
            testResults: {
                ...
                uploadButtonTest: uploadButtonClicked ? '✅ 成功点击上传按钮' : '❌ 未点击任何上传按钮',
                fileDialogTest: fileDialogTriggered || fileDialogOpened ? '✅ 文件选择对话框正常' : '⚠️ 未触发文件选择对话框',
            ...
            uploadElements: pageContent.uploadElements,
        ...
        if (uploadButtonClicked && !fileDialogTriggered && !fileDialogOpened) {
