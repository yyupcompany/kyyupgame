...
const testRoles = [
    ...
    },
    {
        ...
        screenshotPrefix: 'principal_final',
        expectedRole: '园长'
    },
    {
        name: '老师',
        ...
        screenshotPrefix: 'teacher_final',
        expectedRole: '教师'
    },
    {
        name: '家长',
    ...
    try {
        ...
        try {
            ...
            if (!buttonFound) {
                ...
                // 截图 - 显示可用按钮
                const debugScreenshot = path.join(SCREENSHOT_DIR, `${role.screenshotPrefix}_debug_buttons.png`);
                await page.screenshot({ path: debugScreenshot, fullPage: true });
                console.log(`调试截图保存: ${debugScreenshot}`);
                return result;
            }
        } catch (error) {
        ...
        if (finalUrl.includes('/dashboard') || finalUrl.includes('/home') || finalUrl.includes('/index')) {
            ...
            try {
                ...
                if (pageText.includes(role.expectedRole)) {
                    result.actualRole = role.expectedRole;
                    console.log(`确认角色: ${result.actualRole}`);
                } else {
                    // 检查其他可能的角色标识
                    const roleKeywords = [role.expectedRole, role.name];
                    for (const keyword of roleKeywords) {
                        if (pageText.includes(keyword)) {
                            result.actualRole = role.expectedRole;
                            console.log(`通过关键词确认角色: ${keyword}`);
                            break;
                        }
                    }
            ...
            if (finalUrl.includes('/login')) {
                result.errorMessage = '登录失败，仍在登录页面';
                ...
                try {
                    ...
                    console.log('无法获取错误信息');
                }
            } else {
                result.errorMessage = `登录后重定向到意外页面: ${finalUrl}`;
...
function generateReport(results) {
    const report = {
        ...
        summary: {
            ...
            passwordUnificationTests: results.filter(r => r.password === '123456').length
        },
        results: results,
        testConclusion: {
            passwordUnified: results.every(r => r.password === '123456'),
            allRolesWork: results.filter(r => r.success).length === results.length,
            needsAttention: results.filter(r => !r.success).map(r => r.role)
        },
        screenshots: fs.readdirSync(SCREENSHOT_DIR).filter(file => file.endsWith('.png'))
    };

    // 保存报告
    const reportPath = path.join(__dirname, 'final_login_test_report.json');
...
async function runTests() {
    console.log('开始最终快捷登录功能测试...');
    console.log(`测试目标: ${BASE_URL}`);
    console.log('='.repeat(60));
    console.log('测试重点: 验证统一密码123456和快捷登录功能');
    console.log('='.repeat(60));

    // 逐一测试每个角色
    for (const role of testRoles) {
        const result = await testRoleLogin(role);
    ...
    console.log(`密码统一: ${report.summary.passwordUnificationTests}/${report.summary.totalTests}`);
    console.log('='.repeat(60));

    console.log('\n快捷登录功能测试结论:');
    console.log(`✅ 密码统一: ${report.testConclusion.passwordUnified ? '是' : '否'}`);
    console.log(`✅ 所有角色工作正常: ${report.testConclusion.allRolesWork ? '是' : '否'}`);
    if (report.testConclusion.needsAttention.length > 0) {
        console.log(`⚠️  需要关注的角色: ${report.testConclusion.needsAttention.join(', ')}`);
    }
    ...
    testResults.forEach(result => {
        ...
        console.log(`  密码统一: ${result.password === '123456' ? '是 ✓' : '否 ✗'}`);
