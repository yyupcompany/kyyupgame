 ...
 * - 目标号码: 18611141133
 * 
 * 用法: node tests/make-call-with-gateway-route.js
 */
...
const CALL_CONFIG = {
  ...
  authUsername: 'kanderadmin',  // 认证账号
  authPassword: 'Szblade3944',
  gatewayName: 'sales001',      // 呼叫网关
 ...
 * 使用网关路由拨打电话
 */
async function makeCallWithGatewayRoute() {
  console.log('\n🚀 使用网关路由拨打电话\n');
  ...
  try {
    ...
    // 步骤2: 拨打电话（使用Route头指定网关）
    console.log('步骤2: 拨打电话（使用Route头指定网关）');
    testState.callId = `call_${Date.now()}`;
    testState.startTime = new Date();

    const authUri = `sip:${CALL_CONFIG.authUsername}@${CALL_CONFIG.sipServer}`;
    const targetUri = `sip:${CALL_CONFIG.phoneNumber}@${CALL_CONFIG.sipServer}`;
    const gatewayUri = `sip:${CALL_CONFIG.gatewayName}@${CALL_CONFIG.sipServer}`;
    const fromTag = Math.random().toString(36).substring(7);

    console.log(`   认证URI: ${authUri}`);
    console.log(`   目标URI: ${targetUri}`);
    console.log(`   网关URI: ${gatewayUri}`);
    ...
    const inviteRequest = {
      ...
      headers: {
        ...
        contact: [{ uri: authUri }],
        route: [{ uri: gatewayUri }],  // 🔑 关键：指定网关路由
    ...
    console.log('   发送INVITE（带Route头）...');
    console.log(`   Route: ${gatewayUri}\n`);
 ...
 */
async function main() {
  await makeCallWithGatewayRoute();
