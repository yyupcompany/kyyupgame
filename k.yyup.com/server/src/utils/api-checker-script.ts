...
// 从服务端路由自动提取API列表
const extractApisFromRoutes = (): ApiDefinition[] => {
...
// 检测API可用性
async function checkApiAvailability(baseUrl = process.env.SERVER_URL || 'https://shlxlyzagqnc.sealoshzh.site'): Promise<ApiCheckResult[]> {
...
// 生成API可用性报告
async function generateApiReport(baseUrl: string): Promise<void> {
