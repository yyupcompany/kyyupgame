 ...
 * @returns Joi验证对象
 */
const marketingCampaignSchema = (isUpdate = false) => {
  // 基础验证规则
  const baseSchema = {
  ...
  if (isUpdate) {
    return Joi.object({
      ...Object.entries(baseSchema).reduce((acc, [key, schema]) => {
        acc[key] = (schema as any).optional();
        return acc;
 ...
 * @returns 验证结果
 */
export const validateMarketingCampaign = (data: unknown, isUpdate = false) => {
  const schema = marketingCampaignSchema(isUpdate);
 ...
 * @returns 验证结果
 */
export const validateMarketingCampaignRules = (data: unknown) => {
