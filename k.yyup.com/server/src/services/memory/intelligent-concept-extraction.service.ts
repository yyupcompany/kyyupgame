    ...
    try {
      ...
      if (!this.flashModelConfig) {
        ...
        logger.info('[智能概念提取] Flash模型配置加载成功', {
          model: (this.flashModelConfig as any).displayName || this.flashModelConfig.name
        });
      }
    } catch (error) {
...
请确保提取的概念准确、分类合理，并给出适当的置信度评估。`;
   ...
   * 回退的概念提取方法（基于规则）
   */
  private fallbackConceptExtraction(text: string): ConceptExtractionResult {
    logger.info('[智能概念提取] 使用回退提取方法');
