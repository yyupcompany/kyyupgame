...
import { EnrollmentApplicationMaterial, MaterialStatus } from '../../models/enrollment-application-material.model';
...
export class EnrollmentApplicationService {
   ...
   * @returns 创建的报名申请
   */
  async createApplication(applicationData: InferCreationAttributes<EnrollmentApplication>, userId: number): Promise<EnrollmentApplication> {
    const transaction = await sequelize.transaction();
    
    try {
      // 检查家长是否存在
      const parent = await ParentStudentRelation.findByPk(applicationData.parentId as number);
      if (!parent) {
        throw new ApiError(404, '家长不存在');
      }
      
      // 检查招生计划是否存在
      const plan = await EnrollmentPlan.findByPk(applicationData.planId as number);
      if (!plan) {
   ...
   * @returns 更新后的报名申请
   */
  async updateApplication(id: number, applicationData: Partial<InferCreationAttributes<EnrollmentApplication>>, userId: number): Promise<EnrollmentApplication> {
    const transaction = await sequelize.transaction();
    ...
    try {
      ...
      if (applicationData.parentId && applicationData.parentId !== application.parentId) {
        const parent = await ParentStudentRelation.findByPk(applicationData.parentId);
        if (!parent) {
          throw new ApiError(404, '家长不存在');
        }
      }
      ...
      // 更新报名申请 (移除 updatedBy)
      await application.update(applicationData, { transaction });
  ...
  async getApplications(filters: any, page: number, size: number): Promise<any> {
    ...
    const condition: any = {
      deletedAt: { [Op.eq]: null } // 只查询未删除的记录
    };
   ...
   * @returns 审核后的报名申请
   */
  async reviewApplication(id: number, reviewData: Partial<InferCreationAttributes<EnrollmentApplication>>, userId: number): Promise<EnrollmentApplication> {
    const transaction = await sequelize.transaction();
