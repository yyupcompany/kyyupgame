 ...
 * 插入sales001 SIP配置脚本
 * 
 * 用法: node scripts/insert-sales001-sip-config.js
 */

const mysql = require('mysql2/promise');
require('dotenv').config();

async function insertSales001SIPConfig() {
  let connection;
  ...
  try {
    ...
    // 删除已存在的sales001配置
    console.log('🗑️  删除已存在的sales001配置...');
    await connection.query(`
      DELETE FROM sip_configs WHERE username = 'sales001'
    `);
    console.log('✅ 已删除旧配置\n');

    // 插入sales001 SIP配置
    console.log('📝 插入sales001 SIP配置...');
    ...
    console.log('✅ sales001 SIP配置插入成功\n');
    ...
    console.error('\n❌ 插入sales001 SIP配置失败:', error.message);
    console.error(error);
    process.exit(1);
  } finally {
    if (connection) {
