...
export class EnrollmentPlanService {
  ...
  public async create(data: CreateEnrollmentPlanDto, userId: number): Promise<EnrollmentPlan> {
    ...
    try {
      ...
      const existingPlan = await EnrollmentPlan.findOne({
        where: {
          year: data.year,
          semester: data.term === '春季' ? 1 : 2 // 将term转换为semester
        },
        transaction
      });
      ...
      const plan = await EnrollmentPlan.create({
        ...
        targetAmount: 0, // 默认值
        ageRange: '3-6岁', // 默认值
        status: data.status === 'draft' ? 0 : (data.status === 'active' ? 1 : (data.status === 'completed' ? 2 : 3)), // 状态转换
      ...
      if (classIds.length > 0) {
        ...
        await EnrollmentPlanClass.bulkCreate(classAssociations, { transaction });
      }
      ...
      if (data.assigneeIds && data.assigneeIds.length > 0) {
        ...
        await EnrollmentPlanAssignee.bulkCreate(assigneeAssociations, { transaction });
      }
  ...
  public async list(filters: EnrollmentPlanFilterParams): Promise<EnrollmentPlanListResponse> {
    ...
    if (term) {
      where.semester = term === '春季' ? 1 : 2; // 将term转换为semester
    }
    
    if (status) {
      where.status = status === 'draft' ? 0 : (status === 'active' ? 1 : (status === 'completed' ? 2 : 3)); // 状态转换
    }
    ...
    const items = rows.map(item => {
      ...
      return {
        ...
        term: planItem.semester === 1 ? '春季' : '秋季', // 将semester转换为term
        status: planItem.status === 0 ? 'draft' : (planItem.status === 1 ? 'active' : (planItem.status === 2 ? 'completed' : 'cancelled')), // 状态转换
        ...
        completionRate,
        createdAt: planItem.createdAt.toISOString(),
        updatedAt: planItem.updatedAt.toISOString()
      };
    });
  ...
  public async detail(id: number): Promise<EnrollmentPlanDetailResponse> {
    ...
    return {
      ...
      term: planData.semester === 1 ? '春季' : '秋季', // 将semester转换为term
      status: planData.status === 0 ? 'draft' : (planData.status === 1 ? 'active' : (planData.status === 2 ? 'completed' : 'cancelled')), // 状态转换
  ...
  public async update(id: number, data: UpdateEnrollmentPlanDto): Promise<EnrollmentPlan> {
    ...
    try {
      ...
      // 如果修改年份或学期，需要检查是否有冲突
      if ((data.year && data.year !== plan.year) || (data.term && (data.term === '春季' ? 1 : 2) !== plan.semester)) {
        const existingPlan = await EnrollmentPlan.findOne({
          where: {
            year: data.year || plan.year,
            semester: data.term ? (data.term === '春季' ? 1 : 2) : plan.semester,
            id: { [Op.ne]: id }
          },
          transaction
        });
        ...
        if (existingPlan) {
          throw ApiError.badRequest(`${data.year || plan.year}年${data.term || (plan.semester === 1 ? '春季' : '秋季')}已有招生计划`, 'PLAN_EXISTS');
        }
      }
      ...
      if (data.term !== undefined) updateData.semester = data.term === '春季' ? 1 : 2;
      if (data.startDate !== undefined) updateData.startDate = new Date(data.startDate);
      if (data.endDate !== undefined) updateData.endDate = new Date(data.endDate);
      if (data.targetCount !== undefined) updateData.targetCount = data.targetCount;
      if (data.status !== undefined) {
        updateData.status = data.status === 'draft' ? 0 : (data.status === 'active' ? 1 : (data.status === 'completed' ? 2 : 3));
      }
  ...
  public async delete(id: number): Promise<boolean> {
    ...
    try {
      ...
      // 如果计划已开始招生（状态为active或completed），不允许删除
      if (plan.status === 1 || plan.status === 2) {
        throw ApiError.badRequest('已开始或已完成的计划不能删除', 'PLAN_NOT_DELETABLE');
      }
      ...
      // 删除执行跟踪记录
      await EnrollmentPlanTracking.destroy({
        where: { planId: id },
        transaction
      });
  ...
  public async addTracking(planId: number, data: any, userId: number): Promise<EnrollmentPlanTracking> {
    ...
    try {
      ...
      // 检查招生计划状态
      if (plan.status !== 1) { // 1表示'active'状态
      ...
      // 更新计划的实际招生人数 - 由于模型中没有actualCount字段，这里需要修改
      // 这里应该更新一个实际存在的字段或者通过其他方式记录实际招生人数
      // 例如可以使用备注字段或者通过关联表来计算
      // 暂时注释掉这行代码
      // await plan.update({
      //   actualCount: Sequelize.literal(`actual_count + ${data.count}`)
      // }, { transaction });
