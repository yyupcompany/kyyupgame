...
describe('家长相关API严格验证', () => {
  ...
  test('GET /api/parent-assistant - 家长助手格式标准', async () => {
    
    if (!authToken) {
      console.log('⚠️ 跳过家长助手测试：未获取到认证token');
      return;
    }
  ...
  test('GET /api/parent-student-relations - 家长学生关系格式标准', async () => {
    
    if (!authToken) {
      console.log('⚠️ 跳过家长学生关系测试：未获取到认证token');
      return;
    }
  ...
  describe('📊 家长相关API测试报告', () => {
    test('生成家长相关API测试覆盖报告', async () => {
      const endpoints = [
