...
describe('广告相关API严格验证', () => {
  ...
  test('GET /api/advertisements - 广告列表格式标准', async () => {
    
    if (!authToken) {
      console.log('⚠️ 跳过广告列表测试：未获取到认证token');
      return;
    }
  ...
  test('GET /api/advertisement - 广告详情格式标准', async () => {
    
    if (!authToken) {
      console.log('⚠️ 跳过广告详情测试：未获取到认证token');
      return;
    }
  ...
  describe('📊 广告相关API测试报告', () => {
    test('生成广告相关API测试覆盖报告', async () => {
      const endpoints = [
        '/api/advertisements',
        '/api/advertisement'
      ];
