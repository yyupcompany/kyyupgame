...
app.get('/api/training/activities', async (req, res) => {
  try {
    ...
    if (targetAge) {
      whereCondition[sequelize.op.and] = sequelize.where(
        sequelize.col('targetAgeMin'),
        '<=',
        parseInt(targetAge)
      );
      whereCondition[sequelize.op.and] = sequelize.where(
        sequelize.col('targetAgeMax'),
        '>=',
        parseInt(targetAge)
      );
    }
...
async function startServer() {
  try {
    ...
    app.listen(PORT, () => {
      ...
      console.log(`curl http://localhost:${PORT}/api/training/activities`);
    });
  } catch (error) {
