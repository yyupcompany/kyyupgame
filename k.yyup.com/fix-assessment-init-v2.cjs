...
// 1. 在导入部分添加测评模型导入
const importInsertPosition = content.indexOf('// 添加游戏系统模型');
if (importInsertPosition === -1) {
  console.log('❌ 无法找到合适的导入插入位置');
  process.exit(1);
}

const assessmentImports = `// 添加测评系统模型
...
// 找到该行的结束位置（找到下一个 console.log）
const nextLogPos = content.indexOf('console.log', gameInitEnd);
if (nextLogPos === -1) {
  console.log('❌ 无法找到下一个日志位置');
  process.exit(1);
}

content = content.slice(0, nextLogPos) + assessmentInit + content.slice(nextLogPos);
