...
import KindergartenRewardsService, {
  type KindergartenReward,
  type RewardStats
} from '@/api/modules/kindergarten-rewards'

const router = useRouter()

// 响应式数据
const loading = ref(false)
const rewards = ref<KindergartenReward[]>([])
const filterStatus = ref('')
const filterType = ref('')
const detailDialogVisible = ref(false)
const selectedReward = ref<KindergartenReward | null>(null)
const useVoucherDialogVisible = ref(false)
const selectedVoucher = ref<KindergartenReward | null>(null)
const useVoucherLoading = ref(false)

// 统计数据
const stats = reactive<RewardStats>({
  availableRewards: 0,
  usedRewards: 0,
  expiredRewards: 0,
  totalRewards: 0,
...
const refreshRewards = async () => {
  ...
  try {
    // 调用真实API获取奖励数据和统计信息
    const { rewards: rewardsData, stats: statsData } = await KindergartenRewardsService.refreshRewardsData({
...
@use '@/styles/tokens/index.scss' as tokens;
