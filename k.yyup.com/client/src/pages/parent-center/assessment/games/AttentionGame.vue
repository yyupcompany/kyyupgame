    ...
    <div class="game-area">
      <div class="images-container">
        <div class="image-wrapper">
          <canvas ref="canvasLeftRef" :width="gameCanvasSize" :height="gameCanvasSize"></canvas>
          <div class="image-label">图片A</div>
        </div>
        <div class="image-wrapper">
          <canvas ref="canvasRightRef" :width="gameCanvasSize" :height="gameCanvasSize"></canvas>
...
const initDifferences = () => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const margin = canvasSize * 0.125 // 50px 相对于 400px 的比例
    const maxOffset = canvasSize * 0.05 // 20px 相对于 400px 的比例

    let leftX = Math.random() * (canvasSize - 2 * margin) + margin
    let leftY = Math.random() * (canvasSize - 2 * margin) + margin
    let rightX = leftX + (Math.random() * maxOffset * 2 - maxOffset) // 稍微偏移
...
const drawImage = (canvas: HTMLCanvasElement, side: 'left' | 'right', foundIds: number[]) => {
  ...
  gradient.addColorStop(0, getComputedStyle(document.documentElement).getPropertyValue('--border-color-light').trim() || 'var(--color-primary-500)')
  gradient.addColorStop(1, getComputedStyle(document.documentElement).getPropertyValue('--bg-secondary').trim() || 'var(--color-primary-500)')
  ctx.fillStyle = gradient
  ctx.fillRect(0, 0, canvas.width, canvas.height)

  // 绘制一些装饰元素（模拟图片内容）
  ctx.fillStyle = getComputedStyle(document.documentElement).getPropertyValue('--text-placeholder').trim() || 'var(--color-primary-500)'
  const canvasSize = canvas.width
  const decorationScale = canvasSize / 400 // 基于400px的比例缩放

  ctx.fillRect(50 * decorationScale, 50 * decorationScale, 100 * decorationScale, 80 * decorationScale)
  ctx.fillRect(200 * decorationScale, 150 * decorationScale, 120 * decorationScale, 90 * decorationScale)
  ctx.fillRect(100 * decorationScale, 250 * decorationScale, 80 * decorationScale, 100 * decorationScale)
  ...
  sideDifferences.forEach(diff => {
    if (foundIds.includes(diff.id)) {
      // 已找到的差异点 - 绿色圆圈
      ctx.fillStyle = getComputedStyle(document.documentElement).getPropertyValue('--success-color').trim() || 'var(--color-primary-500)'
      ctx.beginPath()
      ctx.arc(diff.x, diff.y, 25, 0, Math.PI * 2)
      ctx.fill()
      ctx.strokeStyle = getComputedStyle(document.documentElement).getPropertyValue('--bg-color').trim() || 'var(--color-primary-500)'
      ctx.lineWidth = 3
      ctx.stroke()
      // 添加对勾
      ctx.strokeStyle = getComputedStyle(document.documentElement).getPropertyValue('--bg-color').trim() || 'var(--color-primary-500)'
      ...
      // 未找到的差异点 - 半透明提示
      ctx.fillStyle = getComputedStyle(document.documentElement).getPropertyValue('--warning-light-bg').trim() || 'var(--color-primary-500)'
      ctx.beginPath()
