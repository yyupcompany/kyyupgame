...
<el-table class="responsive-table" :data="childInfo.assessments" style="width: 100%" border>
    ...
    <el-dialog
      ...
      width="600px"
    >
      <el-form :model="currentMilestone" label-width="120px">
...
import { PARENT_ENDPOINTS, STUDENT_ENDPOINTS } from '@/api/endpoints';
...
export default defineComponent({
  ...
  setup() {
    ...
    const fetchChildInfo = async () => {
      ...
      // 如果还是没有ID，尝试从localStorage获取（如果之前选择过孩子）
      if (!id || isNaN(id)) {
      ...
      if (!id || isNaN(id)) {
        ...
        try {
          // 使用学生列表API，因为家长的孩子就是学生
          const childrenResponse = await get(STUDENT_ENDPOINTS.BASE, {
            params: { parentId }
          });
          ...
          if (childrenResponse.success && childrenResponse.data) {
            ...
            if (children.length === 0) {
              ElMessage.warning('您还没有关联的孩子，请先添加孩子');
      ...
      if (!id || isNaN(id)) {
        ElMessage.error('孩子ID不能为空，请先选择孩子');
        router.push('/parent-center/children');
        return;
      }
      ...
      try {
        // 使用学生成长报告API
        const response = await get(`${STUDENT_ENDPOINTS.BASE}/${childId.value}/growth-report`);
