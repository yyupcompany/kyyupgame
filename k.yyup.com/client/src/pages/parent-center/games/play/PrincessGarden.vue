...
// 场景图片（根据关卡动态选择）
const sceneImages = [
  { left: buildSceneUrl('princess-garden/magic-castle-A.png'), right: buildSceneUrl('princess-garden/magic-castle-B.png') },
  { left: buildSceneUrl('princess-garden/flower-garden-A.png'), right: buildSceneUrl('princess-garden/flower-garden-B.png') },
  { left: buildSceneUrl('princess-garden/tea-party-A.png'), right: buildSceneUrl('princess-garden/tea-party-B.png') },
  { left: buildSceneUrl('princess-garden/fairy-forest-A.png'), right: buildSceneUrl('princess-garden/fairy-forest-B.png') },
  { left: buildSceneUrl('princess-garden/royal-bedroom-A.png'), right: buildSceneUrl('princess-garden/royal-bedroom-B.png') }
]
...
onMounted(() => {
  ...
  // 播放BGM
  audioManager.playBGM(buildBGMUrl('princess-garden-bgm.mp3'))
})
...
const playVoice = (type: string) => {
  ...
  if (type === 'correct') {
    ...
    voicePath = buildVoiceUrl(`correct-${randomNum}.mp3`, 'princess-garden')
  } else if (type === 'wrong') {
    const randomNum = Math.floor(Math.random() * 2) + 1
    voicePath = buildVoiceUrl(`wrong-${randomNum}.mp3`, 'princess-garden')
  } else if (type === 'hint') {
    const randomNum = Math.floor(Math.random() * 3) + 1
    voicePath = buildVoiceUrl(`hint-${randomNum}.mp3`, 'princess-garden')
  } else if (type === 'level-complete') {
    const randomNum = Math.floor(Math.random() * 3) + 1
    voicePath = buildVoiceUrl(`level-complete-${randomNum}.mp3`, 'princess-garden')
  } else if (type === 'encourage') {
    const randomNum = Math.floor(Math.random() * 3) + 1
    voicePath = buildVoiceUrl(`encourage-${randomNum}.mp3`, 'princess-garden')
  } else {
    const voiceMap: Record<string, string> = {
      'game-start': 'game-start.mp3',
      'three-stars': 'three-stars.mp3',
      'two-stars': 'two-stars.mp3',
      'one-star': 'one-star.mp3'
    }
    const fileName = voiceMap[type]
    if (fileName) {
      voicePath = buildVoiceUrl(fileName, 'princess-garden')
    }
  }
...
.game-title {
  ...
  h2 {
    ...
    color: var(--game-primary-pink);
    padding: 0;
    text-shadow: var(--spacing-xs) 2px var(--spacing-xs) var(--color-primary-500);
  }
...
.image-panel {
  ...
  border-radius: var(--text-2xl);
  padding: var(--text-2xl);
  box-shadow: 0 var(--spacing-sm) 30px var(--color-primary-500);
  transition: all 0.3s ease;

  &:hover {
    transform: translateY(-5px);
    box-shadow: 0 var(--text-sm) 40px var(--color-primary-500);
  }
...
.difference-marker {
  position: absolute;
  border: var(--spacing-xs) dashed transparent;
  border-radius: 50%;
  cursor: pointer;
  transition: all 0.3s ease;
  display: flex;
...
.success-particles {
  ...
  .particle {
    ...
    padding: 0;
    padding: 0;
    background: radial-gradient(circle, var(--game-star-gold), var(--game-primary-pink));
    border-radius: 50%;
...
@keyframes float {
  0%, 100% { transform: translateY(var(--position-none)); }
...
@keyframes hint-pulse {
  ...
  50% {
    border-color: var(--game-primary-pink);
    transform: scale(1.1);
    box-shadow: 0 0 var(--text-2xl) var(--color-primary-500);
  }
}

@keyframes item-bounce {
  0%, 100% { transform: translateY(var(--position-none)); }
  50% { transform: translateY(var(--position-negative-2xl)); }
}
