...
const playSound = (type: string) => {
  const audio = new Audio(buildSFXUrl(`${type}.mp3`))
  audio.volume = GameConfigManager.getVolumeConfig().backgroundMusic.volume
  audio.play().catch(() => {})
}

const playVoice = (type: string) => {
  const audio = new Audio(buildVoiceUrl(`${type}.mp3`, 'color-sorting'))
...
.help-content {
  ...
  .help-section { margin-bottom: var(--text-3xl);
    h3 { font-size: var(--text-xl); color: var(--text-primary); margin: 0 0 var(--text-sm) 0; padding-bottom: var(--spacing-sm); border-bottom: var(--border-width-base) solid var(--border-color-light); }
    ...
    .tip { margin-top: var(--spacing-sm); padding: var(--spacing-sm) var(--spacing-md); background: var(--warning-light-bg); border-left: var(--spacing-xs) solid var(--warning-color); color: var(--warning-color); font-size: var(--text-base); border-radius: var(--spacing-xs); }
    &.tips { background: var(--bg-card); padding: var(--spacing-lg); border-radius: var(--spacing-sm); border: var(--border-width-sm) solid var(--warning-color);
...
.color-sorting-game {
  min-height: 100vh;
  background: linear-gradient(135deg, var(--warning-light-bg) 0%, var(--warning-color-light) 50%, var(--warning-color) 100%);
...
.game-title {
  ...
  h2 {
    ...
    color: var(--warning-color);
    text-shadow: var(--spacing-xs) var(--spacing-xs) var(--spacing-xs) var(--warning-color-glow);
  }
...
.conveyor-belt {
  position: relative;
  height: var(--size-8xl);
  background: linear-gradient(to bottom, var(--text-primary), var(--text-secondary), var(--text-primary));
  border-radius: var(--radius-lg);
  margin-bottom: var(--spacing-3xl);
  overflow: hidden; text-overflow: ellipsis; white-space: nowrap;
  box-shadow: inset 0 var(--spacing-xs) var(--spacing-lg) var(--shadow-inner);
  ...
  .belt-line {
    ...
    background: repeating-linear-gradient(90deg, var(--warning-color) 0px, var(--warning-color) var(--spacing-2xl), transparent var(--spacing-2xl), transparent var(--spacing-4xl));
  ...
  .conveyor-item {
    ...
    cursor: grab;
    transition: all var(--game-transition-fast) var(--ease-out);
    ...
    .item-icon {
      ...
      filter: drop-shadow(0 var(--spacing-xs) var(--spacing-xs) var(--shadow-color));
    }
  }
}
...
.baskets-container {
  display: grid;
  grid-template-columns: repeat(auto-fit, minmax(var(--size-6xl), 1fr));
  gap: var(--spacing-2xl);
  margin-bottom: var(--spacing-3xl);
}
...
.basket {
  ...
  text-align: center;
  transition: all var(--game-transition-normal) var(--ease-out);
...
@keyframes belt-move {
  ...
  100% { background-position: var(--spacing-4xl) 0; }
}
</style>
