...
<el-table class="responsive-table" :data="accountList" v-loading="loading" stripe>
    ...
    >
      ...
      <div v-if="viewingAccount.callerNumbers && viewingAccount.callerNumbers.length > 0" style="margin-top: var(--text-2xl);">
        <h4>电话号码列表</h4>
        <el-table class="responsive-table" :data="viewingAccount.callerNumbers" size="small">
    ...
    >
      <div v-if="managingAccount">
        ...
        <el-table class="responsive-table" :data="managingAccount.callerNumbers" size="small">
          <el-table-column prop="phoneNumber" label="电话号码" />
          <el-table-column prop="displayName" label="显示名称" />
          <el-table-column prop="numberType" label="类型">
            <template #default="{ row }">
