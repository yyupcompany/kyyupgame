<template>
  ...
  >
    ...
    <el-alert
      ...
      type="error"
      :title="`有${overduePlans.length}个检查计划已逾期，请尽快处理`"
      show-icon
      :closable="false"
      class="overdue-alert"
    >
      ...
      <template #header>
        <div class="card-header">
          <div class="header-left">
            <span class="card-title"><UnifiedIcon name="calendar" /> 年度检查时间轴</span>
          ...
          <div class="header-actions">
            ...
            <!-- 跳转到本月 -->
            <el-button type="primary" size="default" @click="scrollToCurrentMonth" class="month-button">
              <UnifiedIcon name="calendar" />
              本月检查
            </el-button>

            <el-select v-model="selectedYear" @change="loadTimeline" class="year-select">
              <el-option
...
<el-table class="responsive-table" 
    ...
    <el-card class="document-management-card" v-if="showDocumentManagement">
      <template #header>
        <div class="card-header">
          <span><UnifiedIcon name="note-edit" /> 文档管理中心</span>
          <div class="header-actions">
    ...
    >
      <template #title>
        <span><UnifiedIcon name="analytics" /> 检查计划AI分析报告</span>
      </template>
      <div v-if="aiAnalysisResult" class="ai-analysis-content">
        ...
        <el-divider content-position="left"><UnifiedIcon name="warning" /> 风险提示</el-divider>
        <el-alert
        ...
        <el-divider content-position="left"><UnifiedIcon name="clipboard" /> 总结</el-divider>
        <el-card shadow="never">
          <p class="summary-text">{{ aiAnalysisResult.analysis?.summary }}</p>
        </el-card>
      </div>
      <template #footer>
    ...
    >
      <template #title>
        <span><UnifiedIcon name="robot" /> AI文档分析助手</span>
      </template>
      <div v-if="aiAssistResult" class="ai-assist-content">
        ...
        <el-divider content-position="left"><UnifiedIcon name="lightbulb" /> 填写建议</el-divider>
        <el-timeline>
          <el-timeline-item
        ...
        <el-divider content-position="left"><UnifiedIcon name="warning" /> 注意事项</el-divider>
        <el-alert
        ...
        <el-divider content-position="left"><UnifiedIcon name="clipboard" /> 总结</el-divider>
        <el-card shadow="never">
          <p class="summary-text">{{ aiAssistResult.analysis?.summary }}</p>
        </el-card>
      </div>
      <template #footer>
...
const handleUpdateDeadline = async (documentId: number, deadline: string) => {
  try {
    ...
    // 调用后端API更新截止日期
    const response = await request.put(`/api/document-instances/${documentId}`, {
      deadline: deadline
    });
...
const loadDocumentTemplates = async () => {
  try {
    ...
    const response = await request.get('/api/document-templates', {
      params: { pageSize: 100 }
    });
    if (response.success) {
...
const loadDocumentInstances = async () => {
  try {
    ...
    const response = await request.get('/api/document-instances', {
      params: { pageSize: 100 }
    });
    if (response.success) {
  ...
  .overdue-alert {
    ...
    .overdue-list {
      ...
      .overdue-item {
        ...
        border-bottom: var(--border-width-base) dashed var(--danger-color);
