...
<el-table class="responsive-table" :data="marketingCampaigns" stripe>
    ...
    <template #tab-consultations">
      <div class="consultations-content">
        ...
        <div class="consultations-list">
          <el-table class="responsive-table" :data="consultations" stripe>
...
const quickActions = ref([
  { key: 'create-campaign', title: '创建营销活动', description: '创建新的营销推广活动', icon: 'star', color: 'primary' },
...
const fetchMarketingCoupons = async () => {
  try {
    const response = await request.get('/api/marketing/coupons')
    if (response.success && response.data) {
...
const fetchConsultations = async () => {
  try {
    const response = await request.get('/api/marketing/consultations')
    if (response.success && response.data) {
...
.overview-content {
  .marketing-center .welcome-section {
    ...
    border: var(--border-width-base) solid var(--el-color-primary-light-3);
    margin-bottom: var(--spacing-lg);
    color: white;
    box-shadow: 0 var(--spacing-sm) var(--spacing-xl) var(--el-color-primary-light-2);
    ...
    .chart-row {
      ...
      .chart-container {
        ...
        .chart {
          ...
          min-height: var(--chart-height, 300px);
        }
      }
    }
  }
