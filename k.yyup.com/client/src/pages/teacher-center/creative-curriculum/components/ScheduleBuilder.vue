...
async function fetchAllCurriculums() {
  try {
    ...
    if (response.data.code === 200 && response.data.data?.rows) {
      ...
      console.warn('⚠️ API返回数据格式不符:', response.data)
    }
  } catch (error) {
    ...
    table {
      ...
      thead {
        ...
        th {
          ...
          font-weight: 600;
          color: var(--text-primary);
          border-bottom: var(--transform-drop) solid #ddd;
          white-space: nowrap;

          &:nth-child(1) { width: auto; }  // 星期
          &:nth-child(2) { max-max-max-width: 100px; width: 100%; width: 100%; width: 100%; } // 开始时间
          &:nth-child(3) { width: 100px; } // 结束时间
          &:nth-child(4) { max-width: 150px; width: 100%; } // 课程名称
          &:nth-child(5) { width: 100px; } // 教室
          &:nth-child(6) { max-width: 120px; width: 100%; } // 备注
          &:nth-child(7) { width: auto; }  // 操作
        }
      }
