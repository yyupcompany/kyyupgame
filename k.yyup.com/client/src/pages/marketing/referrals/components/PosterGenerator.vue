  ...
  <div class="poster-generator">
    ...
    <div v-if="previewUrl" class="poster-preview">
      ...
      <el-image
        :src="previewUrl"
        fit="contain"
        style="width: 100%; max-min-height: 60px; height: auto"
      >
        <template #error>
...
const loadTemplates = async () => {
  try {
    const response = await request.get('/api/marketing/referrals/poster-templates');
    if (response.data) {
...
const handlePreview = async () => {
  ...
  try {
    const response = await request.post('/api/marketing/referrals/generate-poster', {
      ...form,
      preview: true
    });
