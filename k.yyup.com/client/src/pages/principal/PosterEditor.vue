...
// 图标已经通过 UnifiedIcon 组件使用，无需单独导入
...
const loadTemplateImage = (templateId: string) => {
  // 构建模板图片URL - 使用相对路径通过Vite代理
  const templateImageUrl = `/uploads/posters/poster-template-${templateId}.svg`
...
// 快捷消息
const _sendQuickMessage = (message: string) => {
...
const saveProject = async () => {
  try {
    ...
    const err = error as { action?: string }
...
.ai-poster-editor {
  ...
  .editor-header {
    ...
    background: white;
    border-bottom: var(--z-index-dropdown) solid #e9ecef;
    display: flex;
    align-items: center;
  ...
  .editor-main {
      ...
      .preview-container {
        ...
        .poster-canvas {
          ...
          .image-viewer {
            ...
            .zoom-controls {
              position: absolute;
              top: var(--z-index-sticky);
              right: var(--z-index-sticky);
    ...
    .ai-chat-panel {
      ...
      display: flex;
      flex-direction: column;
      border-left: var(--z-index-dropdown) solid #e9ecef;
      overflow: hidden;
        ...
        .message-item {
          ...
          .message-content {
            ...
            .message-poster {
              ...
              .mini-poster {
                max-max-max-width: 120px; width: 100%; width: 100%;
      ...
      // 控制面板样式
      .control-panel {
        border-top: var(--z-index-dropdown) solid #e9ecef;
        background: var(--bg-gray-light);
        transition: all 0.3s ease;
