...
const contentAnalysis = ref<ContentAnalysis | null>(null)
...
const kindergartenInfo = ref<KindergartenInfo | null>(null)
...
const generateCopywriting = async () => {
  ...
  try {
    ...
    if (result && typeof result === 'string' && result.length > 0) {
      ...
      generatedContent.value = parseAIResponse(result)
      contentAnalysis.value = generateAnalysisFromAI(result)
...
const generateDynamicActivityDescription = () => {
  ...
  // 获取当前季节开场白作为参考
  const _seasonOpening = getCurrentSeasonOpening()
