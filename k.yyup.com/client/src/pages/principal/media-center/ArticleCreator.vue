...
interface GeneratedArticle {
  ...
  content: string
  contentImages: { description: string }[]
  images?: any[]
  imageCount?: number
  generatedImages?: any[]
}
...
const generateArticle = async () => {
  ...
  try {
    ...
    if (result && typeof result === 'string' && result.length > 0) {
      console.log('✅ AI生成图文结果:', result)

      // 解析AI响应并生成内容
      progressText.value = '正在解析生成结果...'
      const parsedArticle = parseAIArticleResponse(result)
