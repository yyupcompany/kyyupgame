...
import { useRealApiData } from '@/composables/useRealApiData';
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    // 使用真实API获取权限树数据
    const { data: permissionTree, loading: permissionLoading, refresh: refreshPermissionTree } = useRealApiData<Permission[]>(
      SYSTEM_ENDPOINTS.PERMISSION_TREE,
      {
        immediate: false, // 不立即加载，在对话框打开时手动触发
        retryCount: 2,
        onError: (error) => {
    ...
    // 删除硬编码的权限树加载函数
    // 现在使用 useRealApiData 自动从真实API获取权限树数据
      try {
        // 这里应该调用实际的API
