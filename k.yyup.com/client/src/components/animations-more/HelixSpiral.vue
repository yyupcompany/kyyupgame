<template>
  <div v-if="show" class="helix-spiral-container">
    ...
    <div class="center-info">
      <h1 ref="titleElement" class="main-title">{{ getDynamicTitle() }}</h1>
      <p ref="subtitleElement" class="subtitle">{{ getDynamicSubtitle() }}</p>
...
const getDynamicTitle = () => {
  ...
  return `欢迎${roleDisplayName}使用智能管理系统`
}

const getDynamicSubtitle = () => {
  if (props.subtitle) return props.subtitle
  return '正在为您构建个性化工作空间'
}
...
const initSpiralItems = () => {
  ...
  spiralItems.value = modules.map((item, index) => ({
    label: item.title,
    icon: getIconForModule(item.icon),
    color: item.color || '#FF006E',
    rotation: index * 45,
    radius: 300,
    tilt: 15,
    opacity: 0
