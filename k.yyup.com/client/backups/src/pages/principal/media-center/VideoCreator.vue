          ...
          <el-form-item v-if="formData.mode === 'image_to_video'" label="首帧图片">
            ...
            >
              ...
              <div v-else class="uploaded-image">
                <img :src="uploadedImage" alt="首帧图片" />
        ...
        <div class="preview-content">
          <div v-if="generating" class="generating-state">
            ...
            <el-progress 
              ...
              :stroke-width="8"
              style="margin-top: var(--text-lg); width: 300px;"
            />
          </div>
  ...
  try {
    ...
    if (result && result.success) {
      ...
      if (formData.value.mode === 'script') {
        ...
        generatedContent.value = parseAIVideoResponse(result.message || result.data)
      } else {
    ...
    if (formData.value.mode === 'text_to_video') {
      ...
      if (uploadedImage.value) {
        ...
        if (typeof uploadedImage.value === 'string' && uploadedImage.value.startsWith('data:')) {
          ...
          // 如果是文件对象，提取URL
          requestBody.imageUrl = uploadedImage.value.url || uploadedImage.value.src
        } else {
          // 如果是字符串URL
...
const callAIExpert = async () => {
  ...
  try {
    ...
    if (token) {
      headers['Authorization'] = `Bearer ${token}`
      console.log('✅ 添加认证头到AI专家请求:', token.substring(0, 20) + '...')
    } else {
      console.warn('⚠️ 没有找到认证token，AI专家请求可能会失败')
    }
...
const buildVideoPrompt = () => {
  ...
  const duration = durationMap[formData.value.duration] || formData.value.duration
  ...
  const mode = modeMap[formData.value.mode] || '脚本创作'
            ...
            .script-sections {
              .script-section {
                ...
                .section-header {
                  ...
                  padding: var(--text-lg);
                  background: #e9ecef;
                  border-bottom: var(--border-width-base) solid #dee2e6;
