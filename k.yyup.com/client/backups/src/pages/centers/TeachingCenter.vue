...
const loadCourseProgressData = async () => {
  try {
    ...
    if (response.success) {
      ...
      return {
        overallProgress: data?.overall_stats?.overall_completion_rate || 0,
        achievementRate: data?.overall_stats?.overall_achievement_rate || 0,
  ...
  return { overallProgress: 0, achievementRate: 0, classList: [] }
}
...
const loadOutdoorTrainingData = async () => {
  try {
    ...
    if (response.success) {
      const data = response.data as any
      return {
        averageRate: data?.overview?.outdoor_training?.average_rate || 0,
        completedWeeks: data?.overview?.outdoor_training?.completed_weeks || 0,
  ...
  return { averageRate: 0, completedWeeks: 0, classList: [] }
}
...
const loadExternalDisplayData = async () => {
  try {
    ...
    if (response.success) {
      ...
      return {
        averageRate: data?.overview?.average_achievement_rate || 0,
        semesterOutings: data?.overview?.semester_total_outings || 0,
        totalOutings: data?.overview?.all_time_total_outings || 0,
        classList: data?.class_statistics || []
      }
    }
  } catch (error) {
  ...
  return { averageRate: 0, semesterOutings: 0, totalOutings: 0, classList: [] }
}
...
const loadChampionshipData = async () => {
  try {
    ...
    if (response.success) {
      const data = response.data
      return {
        brainScienceRate: data?.achievement_rates?.brain_science_plan || 0,
        courseContentRate: data?.achievement_rates?.course_content || 0,
        outdoorTrainingRate: data?.achievement_rates?.outdoor_training_display || 0,
        externalDisplayRate: data?.achievement_rates?.external_display || 0,
...
.timeline-section {
  ...
  .timeline-container {
    ...
    .timeline-content {
      padding: var(--text-lg);
      background: var(--bg-secondary, var(--bg-container));  // ✅ 使用项目统一变量
...
.teaching-drawer {
  :deep(.el-drawer__header) {
    margin-bottom: 0;
    padding: var(--text-2xl) var(--text-2xl) 0;
  }
