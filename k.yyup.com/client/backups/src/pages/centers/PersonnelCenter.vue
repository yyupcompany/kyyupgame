...
import { Plus, View, Edit, Delete, User, ChatDotRound, ArrowDown } from '@element-plus/icons-vue'
...
const tabs = [
  ...
  { key: 'parents', label: '家长管理', icon: 'UserGroup' },
  { key: 'teachers', label: '教师管理', icon: 'UserCheck' },
...
const personnelDistributionChart = ref({
  ...
  // 明亮配色（深色背景下更清晰）
  color: ['#4C7EFF', '#6EE7B7', 'var(--status-warning)', 'var(--status-error)'],
  legend: {
    orient: 'horizontal',
    left: 'center',
    bottom: 30,
  ...
  series: [{
    ...
    data: [
      { value: 280, name: '学生', itemStyle: { color: '#5470c6' } },
      { value: 45, name: '家长', itemStyle: { color: '#91cc75' } },
      { value: 35, name: '教师', itemStyle: { color: '#fac858' } },
      { value: 8, name: '班级', itemStyle: { color: '#ee6666' } }
    ],
    emphasis: {
      itemStyle: {
        shadowBlur: 10,
...
const quickActions = ref([
  ...
  { key: 'add-teacher', title: '新增教师', description: '添加新的教师信息', icon: 'UserCheck', color: 'warning' },
...
const normalizeDistributionOption = (serverOption: any) => {
  ...
  option.legend = {
    ...
    itemHeight: 16,
    itemGap: 20,
    textStyle: { fontSize: 14, color: 'var(--text-secondary)', ...(option.legend?.textStyle || {}) },
    padding: [10, 0]
  }
