      ...
      <div class="table-content">
        ...
        <div class="table-body">
          ...
          >
            ...
            <el-table-column label="所带班级" prop="classNames" width="150">
              <template #default="{ row }">
                <el-tag
                ...
                <span v-if="!row.classNames || row.classNames.length === 0" class="text-muted">未分配</span>
              </template>
            </el-table-column>
            ...
            <el-table-column label="操作" width="220" fixed="right">
              <template #default="{ row }">
                <el-button link type="primary" @click="viewTeacherDetail(row)">查看</el-button>
                <el-button link type="primary" @click="editTeacher(row)">编辑</el-button>
                <el-button link type="warning" @click="assignTeacherToClass(row)">分配班级</el-button>
                <el-button link type="danger" @click="handleDeleteTeacher(row)">删除</el-button>
              </template>
            </el-table-column>
          </el-table>
        </div>
...
const loadTeachers = async () => {
  ...
  try {
    ...
    if (response.success && response.data) {
      ...
      const errorInfo = ErrorHandler.handle(new Error(response.message || '获取教师列表失败'), true)
    }
  } catch (error) {
...
const handleSaveTeacher = async (teacherData: any) => {
  try {
    ...
    if (response.success) {
      ...
      const errorInfo = ErrorHandler.handle(new Error(response.message || '保存失败'), true)
    }
  } catch (error) {
    const errorInfo = ErrorHandler.handle(error, true)
  }
...
const assignTeacherToClass = async (teacher: Teacher) => {
  try {
    ...
    if (response.success) {
      ...
      const errorInfo = ErrorHandler.handle(new Error(response.message || '获取教师班级信息失败'), true)
    }
  } catch (error) {
    const errorInfo = ErrorHandler.handle(error, true)
  }
...
const handleClassAssignConfirm = async (assignData: { teacherId: string; classIds: string[] }) => {
  try {
    ...
    if (response.success) {
      ...
      const errorInfo = ErrorHandler.handle(new Error(response.message || '分配失败'), true)
    }
  } catch (error) {
    const errorInfo = ErrorHandler.handle(error, true)
  }
