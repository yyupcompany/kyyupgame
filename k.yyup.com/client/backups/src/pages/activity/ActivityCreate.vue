    ...
    <div class="step-content">
      ...
      <div v-show="currentStep === 0" class="step-panel">
        <div class="step-header">
          ...
          <!-- 页面感知提示 -->
          <div v-if="pageAwareness.isActive" class="page-awareness-overlay">
            <div class="page-awareness-backdrop" @click="closePageAwareness"></div>
            <div class="page-awareness-tip">
              <el-alert
                :title="pageAwareness.message"
                type="info"
                :closable="true"
                @close="closePageAwareness"
                show-icon
              >
                <template #default>
                  <div class="awareness-content">
                    <p>{{ pageAwareness.message }}</p>
                    <div class="awareness-suggestions" v-if="pageAwareness.suggestions.length > 0">
                      <p><strong>💡 智能建议：</strong></p>
                      <ul>
                        <li v-for="suggestion in pageAwareness.suggestions" :key="suggestion">
                          {{ suggestion }}
                        </li>
                      </ul>
                    </div>
          ...
          <!-- AI填表动画 -->
          <div v-if="aiFillingAnimation.isActive" class="ai-filling-animation">
            <div class="ai-robot">
            ...
            <div class="ai-status">
              <p>{{ aiFillingAnimation.currentAction }}</p>
              <el-progress
                :percentage="aiFillingAnimation.progress"
                :stroke-width="6"
          ...
          <div class="activity-preview">
            ...
            <div class="preview-card">
              ...
              <div class="preview-info">
                ...
                <p><el-icon><Clock /></el-icon> {{ formatTimeRange }}</p>
                <p><el-icon><Location /></el-icon> {{ activityForm.location || '活动地点' }}</p>
                <p><el-icon><User /></el-icon> 限{{ activityForm.capacity || 0 }}人</p>
                <p><el-icon><Money /></el-icon> {{ activityForm.fee ? `¥${activityForm.fee}` : '免费' }}</p>
              </div>
            </div>
          </div>
...
import {
  ...
  Location, User, Money, Star, Loading, ArrowLeft, ArrowRight, Check
...
import type { ImageGenerationRequest } from '@/api/auto-image'
...
const getActivityDetail = async (id: string): Promise<{ success: boolean, data?: ActivityForm, message?: string }> => {
  try {
    const res = await get(ACTIVITY_PLAN_ENDPOINTS.GET_BY_ID(id))
...
const updateActivity = async (id: string, data: ActivityForm): Promise<{ success: boolean, message?: string }> => {
  try {
    const res = await put(ACTIVITY_PLAN_ENDPOINTS.UPDATE(id), data)
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const posterGenerationError = ref('')
    ...
    // 发布到各个渠道
    const publishToChannels = async (activityId?: string) => {
      try {
        const id = activityId || activityId.value
