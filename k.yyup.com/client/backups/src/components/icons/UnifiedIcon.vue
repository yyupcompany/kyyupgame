<template>
  <div class="unified-icon" :class="iconClasses">
    ...
    >
      <!-- 根据当前图标系统渲染图标 -->
      <template v-if="currentIconSystem === 'modern'">
        <path v-if="iconData.path" :d="iconData.path" fill="none" :stroke="currentColor" :stroke-width="strokeWidth" stroke-linecap="round" stroke-linejoin="round"/>
        <g v-if="iconData.elements" v-html="iconData.elements" :stroke="currentColor" fill="none" :stroke-width="strokeWidth" stroke-linecap="round" stroke-linejoin="round"/>
      </template>

      <template v-if="currentIconSystem === 'colorful'">
        <path v-if="iconData.path" :d="iconData.path" :fill="iconData.fillColor || currentColor" stroke="none"/>
        <g v-if="iconData.elements" v-html="iconData.elements" :fill="iconData.fillColor || currentColor"/>
      </template>

      <template v-if="currentIconSystem === 'handdrawn'">
        <path v-if="iconData.path" :d="iconData.path" :stroke="currentColor" fill="none" :stroke-width="strokeWidth + 0.5" stroke-linecap="round" stroke-linejoin="round" :style="{ filter: `url(#roughPaper-${filterId})` }"/>
        <g v-if="iconData.elements" v-html="iconData.elements" :stroke="currentColor" fill="none" :stroke-width="strokeWidth + 0.5" stroke-linecap="round" stroke-linejoin="round" :style="{ filter: `url(#roughPaper-${filterId})` }"/>
      </template>
    </svg>

    <!-- 手绘效果的SVG滤镜 -->
    <svg v-if="currentIconSystem === 'handdrawn'" style="position: absolute; width: 0; height: 0;">
      <defs>
...
const currentIconSystem = computed(() => iconSystem.value)
...
const iconClasses = computed(() => [
  'unified-icon',
  iconSystemClass.value,
  { 'icon-active': props.variant === 'filled' },
...
// 三套图标系统的图标数据 - 扩展原有的图标映射
const iconMap: Record<string, any> = {
  // 原有的UnifiedIcon图标映射（保持兼容性）
  // AI相关图标
  'ai-center': {
    modern: {
      path: 'M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 3c1.66 0 3 1.34 3 3s-1.34 3-3 3-3-1.34-3-3 1.34-3 3-3zm0 14c-2.76 0-5-2.24-5-5s2.24-5 5-5 5 2.24 5 5-2.24 5-5 5z'
    },
    colorful: {
      path: 'M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 3c1.66 0 3 1.34 3 3s-1.34 3-3 3-3-1.34-3-3 1.34-3 3-3zm0 14c-2.76 0-5-2.24-5-5s2.24-5 5-5 5 2.24 5 5-2.24 5-5 5z',
      fillColor: '#9C27B0'
  ...
  'lightning': {
    ...
    handdrawn: {
      path: 'M12 3L4 13H11L10 19L18 9H11L12 3Z'
    }
  },
  ...
  'dashboard': {
    ...
    handdrawn: {
      path: 'M4 4H8V8H4V4ZM12 4H16V8H12V4ZM20 4H24V8H20V4ZM4 12H8V16H4V12ZM12 12H16V16H12V12ZM20 12H24V16H20V12ZM4 20H8V24H4V20ZM12 20H16V24H12V20ZM20 20H24V24H20V20Z',
      elements: '<circle cx="6" cy="14" r="1"/><circle cx="14" cy="14" r="1"/><circle cx="22" cy="14" r="1"/>'
    }
  },
  ...
  'settings': {
    ...
    handdrawn: {
      path: 'M12 10C10 10 8.5 11.5 8.5 13.5S10 17 12 17S15.5 15.5 15.5 13.5S14 10 12 10Z',
      elements: '<path d="M2 12H22M12 2V22M5 5L19 19M19 5L5 19"/>'
    }
  },
  ...
  'finance': {
    ...
    handdrawn: {
      path: 'M12 3.5C7.5 3.5 3.5 7.5 3.5 12C3.5 16.5 7.5 20.5 12 20.5C16.5 20.5 20.5 16.5 20.5 12C20.5 7.5 16.5 3.5 12 3.5Z',
      elements: '<path d="M8 8.5L16 15.5M16 8.5L8 15.5M12 7V17M7 12H17"/>'
    }
  },
  ...
  'task': {
    ...
    handdrawn: {
      path: 'M13 3H7C6 3 5 4 5 5V19C5 20 6 21 7 21H17C18 21 19 20 19V9L13 3Z',
  ...
  'system': {
    modern: {
      path: 'M19.14 12.94c.04-.3.06-.61.06-.94 0-.32-.02-.64-.07-.94l2.03-1.58c.18-.14.23-.41-.12-.61l-1.92-3.32c-.12-.22-.37-.29-.59-.22l-2.39.96c-.5-.38-1.03-.7-1.62-.94L14.4 2.81c-.04-.24-.24-.41-.48-.41h-3.84c-.24 0-.43-.17-.47-.41L9.25 5.35c-.59.24-1.13.56-1.62.94L5.24 5.33c-.22-.08-.47 0-.59.22L2.74 8.87c-.12.21-.08-.47.12.61l2.03 1.58c-.05.3-.09.63-.09.94s.02.64.07.94l-2.03 1.58c-.18.14-.23-.41-.12-.61l1.92-3.32c.12-.22-.07-.47-.12-.61l-2.01-1.58zM12 15.6c-1.98 0-3.6-1.62-3.6-3.6s1.62-3.6 3.6-3.6 3.6 1.62 3.6 3.6-1.62 3.6-3.6 3.6z'
    },
    colorful: {
      path: 'M19.14 12.94c.04-.3.06-.61.06-.94 0-.32-.02-.64-.07-.94l2.03-1.58c.18-.14.23-.41-.12-.61l-1.92-3.32c-.12-.22-.37-.29-.59-.22l-2.39.96c-.5-.38-1.03-.7-1.62-.94L14.4 2.81c-.04-.24-.24-.41-.48-.41h-3.84c-.24 0-.43-.17-.47-.41L9.25 5.35c-.59.24-1.13.56-1.62.94L5.24 5.33c-.22-.08-.47 0-.59.22L2.74 8.87c-.12.21-.08-.47.12.61l2.03 1.58c-.05.3-.09.63-.09.94s.02.64.07.94l-2.03 1.58c-.18.14-.23-.41-.12-.61l1.92-3.32c.12-.22-.07-.47-.12-.61l-2.01-1.58zM12 15.6c-1.98 0-3.6-1.62-3.6-3.6s1.62-3.6 3.6-3.6 3.6 1.62 3.6 3.6-1.62 3.6-3.6 3.6z',
      fillColor: '#FFC107'
  ...
  'search': {
    ...
    colorful: {
      path: 'M15.5 14h-.79l-.28-.27C15.41 12.59 16 11.11 16 9.5 16 5.91 13.09 3 9.5 3S3 5.91 3 9.5 16 5.91 16 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 14 9.5 14z',
      fillColor: '#9E9E9E'
    },
    handdrawn: {
  ...
  'calendar': {
    ...
    handdrawn: {
      path: 'M6 4H18C19 4 20 5 20 6V18C20 19 19 20 18 20H6C5 20 4 19 4 18V6C4 5 5 5 6 5Z',
  ...
  'students': {
    modern: {
      path: 'M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2',
      elements: '<circle cx="9" cy="7" r="4"/><circle cx="15" cy="7" r="4"/>'
    },
    colorful: {
      path: 'M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2',
      elements: '<circle cx="9" cy="7" r="4" fill="#FF9800"/><circle cx="15" cy="7" r="4" fill="#4CAF50"/>'
    },
    handdrawn: {
      path: 'M17 20V18C17 16 15 14 13 14H7C5 14 3 16 3 18V20',
      elements: '<circle cx="8" cy="8" r="3"/><circle cx="16" cy="8" r="3"/>'
    }
  },

  'teacher': {
    modern: {
      path: 'M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-2-2m2 2l-2 2m2-2l6-6m-6 6l-6-6'
    },
    colorful: {
      path: 'M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-2-2m2 2l-2 2m2-2l6-6m-6 6l-6-6',
      fillColor: '#9C27B0'
    ...
    handdrawn: {
      path: 'M12 3C7 3 3 7 3 12S7 21 12 21S21 17 21 12S17 3 12 3Z',
      elements: '<path d="M10 15L12 13L14 15M12 8L14 10L10 12"/>'
    }
  },
  ...
  'classroom': {
    ...
    handdrawn: {
      path: 'M12 3L3 8V17C3 22 7 25 12 26C17 25 21 22 21 17V8L12 3Z',
      elements: '<path d="M7 12L12 7L17 12M12 15V20"/>'
    }
  },

  'phone': {
    modern: {
      path: 'M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 1.42 1.42.63 3.36-1.58 2.83-2.2.72-4.12C13.46 9.19 14 8.63 14 8a6 6 0 0 0-7.92 9.48A9.9 9.9 0 0 0 3.18 4.2c-1.58.5-2.2 1.41-2.83 2.83A2 2 0 0 1 2 19.92v-3A10 10 0 0 1 12 2a10 10 0 0 1 10 14.92z'
    },
    colorful: {
      path: 'M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 1.42 1.42.63 3.36-1.58 2.83-2.2.72-4.12C13.46 9.19 14 8.63 14 8a6 6 0 0 0-7.92 9.48A9.9 9.9 0 0 0 3.18 4.2c-1.58.5-2.2 1.41-2.83 2.83A2 2 0 0 1 2 19.92v-3A10 10 0 0 1 12 2a10 10 0 0 1 10 14.92z',
      fillColor: '#2196F3'
    ...
    handdrawn: {
      path: 'M20 16V19C20 20 19 21 18 21C15 21 12 20 12 20C12 20 9 21 6 21C5 21 4 20 4 19V16',
      elements: '<path d="M12 2C12 2 12 6 12 6M8 10H16"/>'
    }
  },
  ...
  'phone-call': {
    ...
    handdrawn: {
      path: 'M20 16V19C20 20 19 21 18 21C15 21 12 20 12 20C12 20 9 21 6 21C5 21 4 20 4 19V16',
      elements: '<path d="M8 11C8 11 16 11 16 11M12 5V15"/>'
    }
  },
  ...
  'message': {
    ...
    handdrawn: {
      path: 'M20 11C20 16 16 20 12 20C8 20 4 16 4 11C4 6 8 2 12 2C16 2 20 6 20 11Z',
      elements: '<path d="M6 8L18 8M10 14L14 14"/>'
    }
  },

  'photo-camera': {
    modern: {
      path: 'M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 4.5v7.08L7 14l5-2.21V6.5z'
    },
    colorful: {
      path: 'M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 4.5v7.08L7 14l5-2.21V6.5z',
      fillColor: '#607D8B'
    ...
    handdrawn: {
      path: 'M12 3C7 3 3 7 3 12S7 21 12 21 21 17 21 12 21 7 17 12 3Z',
      elements: '<rect x="7" y="9" width="10" height="6" rx="1"/><path d="M10 6L12 8L14 6"/>'
    }
  },
  ...
  'messages': {
    ...
    colorful: {
      path: 'M20 2H4c-1.1 0-1.99.9-1.99 2L2 22l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm-2 12H6v-2h12v2zm0-3H6V6h12v2zm0-3H6V6h12v2z',
      fillColor: '#2196F3'
    },
    handdrawn: {
  ...
  'script': {
    modern: {
      path: 'M14 2H6c-1.1 0-1.99.9-1.99 2L4 20c0 1.1.89 2 2 2h18c1.1 0 2-.9 2-2V8l-6-6zm2 16H8v-2h8v2zm0-4H8v-2h8v2zm-3-5V3.5L18.5 9H13z'
    },
    colorful: {
  ...
  'monitor': {
    ...
    colorful: {
      path: 'M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l-7-4A2 2 0 0 0 21 16z',
  ...
  'building': {
    modern: {
      path: 'M3 9l9-7 9 7v11H3V9z',
      elements: '<path d="M9 22V12H6V9H3z"/><path d="M12 6V4L8 7l4 3V6z"/>'
    },
    colorful: {
      path: 'M3 9l9-7 9 7v11H3V9z',
      fillColor: '#2196F3'
    ...
    handdrawn: {
      path: 'M3.5 10L12 3L20.5 10V21.5H3.5V10Z',
      elements: '<path d="M8.5 13V11H5.5V10L8.5 13Z"/><path d="M15.5 6V4L12 6.5L15.5 6Z"/>'
    }
  },
  ...
  'home': {
    ...
    handdrawn: {
      path: 'M11 19V13H13V19H18V11H21L12 4L3 11H6V19H11Z'
    }
  },

  'book': {
    modern: {
      path: 'M4 6H2v14c0 1.1.9 2 2 2h14v-2H4V6zm16-4H6c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm-1 14H7V6h12v14z'
    },
    colorful: {
      path: 'M4 6H2v14c0 1.1.9 2 2 2h14v-2H4V6zm16-4H6c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm-1 14H7V6h12v14z',
      fillColor: '#4CAF50'
    ...
    handdrawn: {
      path: 'M5 7H3V20H19V7H17V5H7V7H5Z',
      elements: '<path d="M7 5V7H17V5M8 10H16M8 13H16M8 16H13"/>'
    }
  },

  'trend-charts': {
    modern: {
      path: 'M3.5 18.5l6-6.5 6 6.5L20.5 6.5',
      elements: '<path d="M3 21v-3M7 12v-3M11 18v-3M15 6v-3M19 15v-3"/>'
    },
    colorful: {
      path: 'M3.5 18.5l6-6.5 6 6.5L20.5 6.5',
      fillColor: '#9C27B0'
    ...
    handdrawn: {
      path: 'M4 18L10 11L16 18L21 7',
      elements: '<path d="M3 21L3 18M7 12L7 9M11 19L11 16M15 7L15 4M19 16L19 13"/>'
    }
  },
  ...
  'user-group': {
    ...
    handdrawn: {
      path: 'M16 11C16 9 14.5 7.5 12.5 7.5S9 9 9 11S10.5 14.5 12.5 14.5 16 13 16 11ZM8 11C8 9 6.5 7.5 4.5 7.5S1 9 1 11S2.5 14.5 4.5 14.5 8 13 8 11Z',
      elements: '<path d="M1 19C1 16 4 13.5 6.5 13.5H9.5C12 13.5 15 16 15 19M9 19C9 16 12 13.5 14.5 13.5H17.5C20 13.5 23 16 23 19"/>'
    }
  },
...
const iconData = computed(() => {
  // 首先尝试使用新图标映射，如果没有则使用默认图标
  const fullIconData = iconMap[props.name] || iconMap['default']

  // 根据当前图标系统返回对应的数据
  return fullIconData[currentIconSystem.value] || fullIconData.modern || {}
})

// 兼容性处理 - 如果图标系统中没有找到图标，则回退到简单的路径
const iconPath = computed(() => {
  // 尝试新图标系统中的图标
  if (iconData.value) {
    return ''
  }
  // 如果没有找到，使用原始的kindergartenIcons
  const originalIcon = kindergartenIcons[props.name]
  return originalIcon ? originalIcon.path : ''
})

const hasSecondaryPath = computed(() => {
  // 使用当前图标系统的数据检查是否有二级路径
  return !!iconData.value?.elements
})
...
const variant = computed(() => {
  // 如果是彩色风格，variant参数映射到图标系统
  if (currentIconSystem.value === 'colorful') {
    switch (props.variant) {
      case 'filled': return 'colorful'
      case 'outlined': return 'colorful'
