<template>
  <div class="media-gallery">
    ...
    <div v-else-if="viewMode === 'list' && filteredMedia.length > 0" class="media-list">
      <el-table :data="paginatedMedia" stripe>
...
export default {
  ...
  setup(props) {
    ...
    const getMediaTypeIcon = (type) => {
      const icons = {
        'class_photo': 'el-icon-picture',
        'class_video': 'el-icon-video-camera',
        'student_photo': 'el-icon-picture-outline',
        'student_video': 'el-icon-video-play'
      };
    ...
    const getThumbnailUrl = (media) => {
      if (media?.thumbnail_path) {
        return `http://localhost:3000${media.thumbnail_path}`;
      }
      if (media?.file_path && isPhoto(media)) {
        return `http://localhost:3000${media.file_path}`;
      }
      return '/placeholder-image.jpg'; // 占位图
    };

    const getFullImageUrl = (media) => {
      return media?.file_path ? `http://localhost:3000${media.file_path}` : getThumbnailUrl(media);
    };

    const getVideoUrl = (media) => {
      return media?.file_path ? `http://localhost:3000${media.file_path}` : '';
    };
    ...
    const downloadMedia = (media) => {
      if (media.file_path) {
        ...
        link.href = `http://localhost:3000${media.file_path}`;
