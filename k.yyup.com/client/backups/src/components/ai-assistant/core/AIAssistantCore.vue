...
import { workflowStepManager } from '@/utils/workflow-steps'
  ...
  try {
    ...
    const result = await multiRound.executeMultiRound(message, {
      ...
      context: {
        ...
        pageTitle: currentPageContext.value?.title || route.meta?.title || 'AI助手',
        userRole: role.value,
        role: role.value,
        // 🔧 启用网络搜索：检测消息中是否包含搜索关键词
        enableWebSearch: this.isSearchQuery(message)
      },
    ...
    case 'tool_call_complete':
      ...
      const messages = chatHistory.currentMessages.value
            const toolCallMsg = messages.find(m =>
              m.type === 'tool_call' && m.toolName === completedToolName && m.toolStatus === 'running'
            )

      if (toolCallMsg) {
              // 更新消息状态
              toolCallMsg.toolStatus = event.data?.result?.status === 'success' ? 'completed' : 'failed'
              console.log('✅ [工具调用] 聊天历史状态已更新:', toolCallMsg.toolName, '→', toolCallMsg.toolStatus)
            }
      ...
      if (narration) {
          ...
          chatHistory.addMessage({
            ...
            timestamp: new Date().toISOString()
              ...
              console.log('✅ [工具解说] 已创建工具解说消息:', narrationToolName, narration.substring(0, 50))
          
              // 🎯 同时保存到工具调用的narration字段（用于最终消息汇总）
...
const handleSearchStart = (event: any) => {
  handleEvent({ type: 'search_start', message: event.message, data: event.data })
}

const handleSearchProgress = (event: any) => {
  handleEvent({ type: 'search_progress', message: event.message, data: event.data })
}

const handleSearchComplete = (event: any) => {
  handleEvent({ type: 'search_complete', message: event.message, data: event.data })
}
