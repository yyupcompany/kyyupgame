// 测试 Teacher Center 所有页面的自动化脚本 - 包含硬编码检测
...
(async () => {
  ...
  // Teacher Center 页面列表
  const teacherCenterPages = [
    { url: 'http://localhost:5173/teacher-center', name: 'Teacher Center Dashboard' },
    { url: 'http://localhost:5173/teacher-center/activities', name: 'Activities Management' },
    { url: 'http://localhost:5173/teacher-center/attendance', name: 'Attendance Management' },
    { url: 'http://localhost:5173/teacher-center/creative-curriculum', name: 'Creative Curriculum' },
    { url: 'http://localhost:5173/teacher-center/customer-tracking', name: 'Customer Tracking' },
    { url: 'http://localhost:5173/teacher-center/dashboard', name: 'Dashboard' },
    { url: 'http://localhost:5173/teacher-center/enrollment', name: 'Enrollment Management' },
    { url: 'http://localhost:5173/teacher-center/notifications', name: 'Notifications' },
    { url: 'http://localhost:5173/teacher-center/tasks', name: 'Task Management' },
    { url: 'http://localhost:5173/teacher-center/teaching', name: 'Teaching Management' },
    { url: 'http://localhost:5173/teacher-center/customer-pool', name: 'Customer Pool' }
  ];

  const results = {
    summary: {
      total: teacherCenterPages.length,
      tested: 0,
      successful: 0,
      failed: 0,
      totalErrors: 0,
  ...
  console.log('👨‍🏫 开始测试Teacher Center所有页面（包含硬编码检测）...');

  for (const pageInfo of teacherCenterPages) {
    console.log(`\n📄 测试页面: ${pageInfo.name}`);
  ...
  // 生成测试报告
  const reportPath = `${resultsDir}/teacher-center-test-report-with-hardcoded-check.json`;
  fs.writeFileSync(reportPath, JSON.stringify(results, null, 2));

  // 生成Markdown报告
  const markdownReport = generateMarkdownReport(results, allHardcodedIssues);
  const mdPath = `${resultsDir}/teacher-center-test-report-with-hardcoded-check.md`;
  ...
  if (allHardcodedIssues.length > 0) {
    generateHardcodedReport(allHardcodedIssues, resultsDir, 'teacher-center');
  }
...
function generateMarkdownReport(results, hardcodedIssues) {
  let markdown = `# Teacher Center测试报告（包含硬编码检测）
...
- **硬编码问题**: ${results.summary.totalHardcodedIssues}
  ...
  - 低严重性: ${results.summary.lowSeverityHardcoded}
...
function generateHardcodedReport(hardcodedIssues, resultsDir, centerType) {
  const groupedIssues = {};
  ...
  let report = `# ${centerType}硬编码值检测报告
...
- 考虑使用相对路径`;
  ...
  const reportPath = `${resultsDir}/${centerType}-hardcoded-detection-report.md`;
