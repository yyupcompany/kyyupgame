...
describe('学生管理模块 E2E 测试', () => {
  ...
  test.describe('学生列表功能', () => {
    test('管理员查看学生列表', async ({ page }) => {
      ...
      const extractItems = (json: any): any[] => {
        ...
        if (Array.isArray(json.rows)) return json.rows; // 兼容 {rows, count}
        return [];
      };

      let items: any[] = [];
      const resp = await page.waitForResponse(
        r => r.url().includes('/api/students') && r.request().method() === 'GET',
        { timeout: 8000 }
      ).catch(() => null);
      try {
        if (resp) {
          items = extractItems(await resp.json());
        } else {
          const origin = new URL(BASE_URL).origin;
          const r2 = await page.request.get(`${origin}/api/students`);
          if (r2.ok()) items = extractItems(await r2.json());
        }
      } catch { /* 忽略API解析错误，避免阻断用例 */ }
      ...
      if (items.length > 0) {
        ...
        for (let i = 0; i < sample; i++) {
          ...
          const it = items[i];
          // 学生行包含姓名；若后端字段不同，兼容 name / studentName
          const name = it.name || it.studentName || it.realName;
          expect(rowText && name && rowText.includes(String(name)), `第${i + 1}行应包含后端学生姓名: ${name || ''}`).toBeTruthy();
        }
      }
    });
  ...
  test.describe('权限验证', () => {
    ...
    test('家长无法访问学生管理页面', async ({ page }) => {
      ...
      const parentQuick = page.getByRole('button', { name: /家长/ });
      if (await parentQuick.isVisible().catch(() => false)) {
        await parentQuick.click();
      } else {
        await page.getByPlaceholder('请输入用户名').fill('parent');
...
async function loginAsAdmin(page: Page) {
  ...
  if (await adminQuick.isVisible().catch(() => false)) {
    ...
    await page.getByPlaceholder('请输入用户名').fill(ADMIN_USER.username);
    await page.getByPlaceholder('请输入密码').fill(ADMIN_USER.password);
    await page.getByRole('button', { name: /立即登录|登录/ }).click();
  }
...
async function loginAsTeacher(page: Page) {
  await page.goto(`${BASE_URL}/login`);
  const teacherQuick = page.getByRole('button', { name: /教师/ });
  if (await teacherQuick.isVisible().catch(() => false)) {
    await teacherQuick.click();
  } else {
    await page.getByPlaceholder('请输入用户名').fill(TEACHER_USER.username);
    await page.getByPlaceholder('请输入密码').fill(TEACHER_USER.password);
    await page.getByRole('button', { name: /立即登录|登录/ }).click();
  }
  await expect(page).not.toHaveURL(/\/login$/);
}
