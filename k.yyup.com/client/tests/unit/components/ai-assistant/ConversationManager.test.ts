...
import ConversationManager from '@/components/ai-assistant/ConversationManager.vue'
...
describe('ConversationManager.vue', () => {
  ...
  const createWrapper = (props = {}) => {
    return mount(ConversationManager, {
      props: {
        visible: true,
  ...
  describe('Component Rendering', () => {
    ...
    it('should display drawer header with title and icon', () => {
      wrapper = createWrapper()
    ...
    it('should display action buttons for creating and refreshing', () => {
      wrapper = createWrapper()
    ...
    it('should show close button in header', () => {
      ...
      expect(closeBtn.findComponent({ name: 'el-icon' }).exists()).toBe(true)
    })
  })
  ...
  describe('Props Handling', () => {
    ...
    it('should emit update:visible event when close button is clicked', async () => {
      wrapper = createWrapper()
  ...
  describe('Empty State', () => {
    it('should show empty state when no conversations exist and not loading', () => {
      wrapper = createWrapper()
    ...
    it('should show skeleton loading state when loading conversations', async () => {
      wrapper = createWrapper()
    ...
    it('should hide empty state when loading', async () => {
      wrapper = createWrapper()
  ...
  describe('Conversation List Display', () => {
    ...
    it('should handle conversation with missing title', async () => {
      wrapper = createWrapper()
      ...
      expect(item.find('.title').text()).toBe('未命名会话')
    })

    it('should handle conversation with missing message count', async () => {
      wrapper = createWrapper()
      
      wrapper.setData({
        conversations: [
          { id: '1', title: '会话1' } // Missing messageCount
        ],
        conversationsLoading: false
      })
      
      const item = wrapper.find('.conversation-item')
      expect(item.find('.message-count').text()).toContain('0 条消息')
    })
  })
  ...
  describe('Time Formatting', () => {
    ...
    it('should use current time when both timestamps are missing', () => {
      wrapper = createWrapper()
  ...
  describe('User Interactions', () => {
    ...
    it('should handle create new conversation button click', async () => {
      const { default: mockRequest } = await import('@/utils/request')
      mockRequest.mockResolvedValue({ data: { id: 'new-conversation-id' } })
      
      wrapper = createWrapper()
      
      const createButton = wrapper.findAllComponents({ name: 'el-button' })[0]
    ...
    it('should handle refresh button click', async () => {
      ...
      const refreshButton = wrapper.findAllComponents({ name: 'el-button' })[1]
  ...
  describe('Conversation Operations', () => {
    it('should toggle conversation pin status', async () => {
      ...
      const pinItem = wrapper.findAllComponents({ name: 'el-dropdown-item' })[0]
    ...
    it('should toggle conversation archive status', async () => {
      ...
      const archiveItem = wrapper.findAllComponents({ name: 'el-dropdown-item' })[2]
    ...
    it('should handle conversation renaming', async () => {
      ...
      const renameItem = wrapper.findAllComponents({ name: 'el-dropdown-item' })[1]
  ...
  describe('Loading Conversations', () => {
    ...
    it('should handle API response with different data structures', async () => {
      const { default: mockRequest } = await import('@/utils/request')
      mockRequest.mockResolvedValue({
        data: [
          { id: '1', title: '会话1' },
    ...
    it('should map API response fields correctly', async () => {
        ...
        data: {
          items: [
            { 
              ...
              last_message_at: '2025-09-15T11:00:00Z',
              is_archived: true,
              is_pinned: false
            }
          ]
      ...
      expect(wrapper.vm.conversations[0].id).toBe('1')
      expect(wrapper.vm.conversations[0].messageCount).toBe(5)
      expect(wrapper.vm.conversations[0].updatedAt).toBe('2025-09-15T10:00:00Z')
      expect(wrapper.vm.conversations[0].lastMessageAt).toBe('2025-09-15T11:00:00Z')
      expect(wrapper.vm.conversations[0].isArchived).toBe(true)
      expect(wrapper.vm.conversations[0].isPinned).toBe(false)
    })
  })
  ...
  describe('Error Handling', () => {
    ...
    it('should handle conversation creation errors', async () => {
      ...
      expect(ElMessage.error).toHaveBeenCalledWith('创建会话失败')
    })
  })
  ...
  describe('Accessibility', () => {
    ...
    it('should prevent event propagation on action clicks', async () => {
      ...
      const actionTrigger = wrapper.find('.action-trigger')
