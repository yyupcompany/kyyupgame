...
import DocumentPreview-fixed from '@/components/ai-assistant/document/DocumentPreview-fixed'
...
describe('DocumentPreview-fixed - 完整测试覆盖', () => {
  let wrapper: any
  ...
  beforeEach(() => {
    ...
    wrapper = mount(DocumentPreview-fixed, {
      global: {
        plugins: [ElementPlus],
        stubs: {
          'el-button': true,
  ...
  describe('基础渲染测试', () => {
    ...
    it('应该包含必要的DOM结构', () => {
      // 根据实际组件结构添加断言
      expect(wrapper.find('.documentpreview-fixed').exists()).toBe(true)
    })
  })
