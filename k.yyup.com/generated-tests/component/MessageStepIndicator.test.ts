...
import MessageStepIndicator from '@/components/ai-assistant/chat/MessageStepIndicator'
...
describe('MessageStepIndicator - 完整测试覆盖', () => {
  let wrapper: any
  ...
  beforeEach(() => {
    ...
    wrapper = mount(MessageStepIndicator, {
      global: {
        plugins: [ElementPlus],
        stubs: {
          'el-button': true,
  ...
  describe('基础渲染测试', () => {
    ...
    it('应该包含必要的DOM结构', () => {
      // 根据实际组件结构添加断言
      expect(wrapper.find('.messagestepindicator').exists()).toBe(true)
    })
  })
