...
import SmartPromotionCenter from '@/pages/marketing/smart-promotion/SmartPromotionCenter'
...
describe('SmartPromotionCenter - 完整测试覆盖', () => {
  let wrapper: any
  ...
  beforeEach(() => {
    ...
    wrapper = mount(SmartPromotionCenter, {
      global: {
        plugins: [ElementPlus],
        stubs: {
          'el-button': true,
  ...
  describe('基础渲染测试', () => {
    ...
    it('应该包含必要的DOM结构', () => {
      // 根据实际组件结构添加断言
      expect(wrapper.find('.smartpromotioncenter').exists()).toBe(true)
    })
  })
