...
class ProgressivePCLoginTest {
  constructor() {
    ...
    this.testConfig = {
      ...
      username: 'admin',
      password: 'admin123',
      description: 'PC端管理员登录测试'
    };
  }

  async init() {
    console.log('🚀 初始化渐进式PC端压力测试环境...');
  ...
  async performLogin(userId) {
    const context = await this.browser.newContext({
      userAgent: `ProgressivePCTest-${userId}`,
    ...
    try {
      console.log(`🔄 用户 ${userId} 开始PC端管理员登录测试`);
      ...
      for (const selector of loginSelectors) {
        try {
          ...
          if (loginElement) {
            console.log(`🎯 用户 ${userId} 找到登录元素: ${selector}`);
            break;
          }
        } catch (e) {
          // 继续尝试下一个选择器
      ...
      if (!loginElement) {
        ...
        console.log(`🔍 用户 ${userId} 尝试直接访问登录页面`);
        ...
        for (const selector of loginSelectors) {
          try {
            ...
            loginElement = await page.$(selector);
            if (loginElement) {
              console.log(`🎯 用户 ${userId} 在登录页面找到登录元素: ${selector}`);
              break;
            }
          } catch (e) {
            // 继续尝试下一个选择器
      ...
      for (const selector of usernameSelectors) {
        try {
          ...
          if (usernameInput) {
            console.log(`📝 用户 ${userId} 找到用户名输入框: ${selector}`);
            break;
          }
        } catch (e) {
          // 继续尝试下一个选择器
      ...
      if (usernameInput) {
        await usernameInput.fill(this.testConfig.username);
      ...
      for (const selector of passwordSelectors) {
        try {
          ...
          if (passwordInput) {
            console.log(`🔒 用户 ${userId} 找到密码输入框: ${selector}`);
            break;
          }
        } catch (e) {
          // 继续尝试下一个选择器
      ...
      if (passwordInput) {
        await passwordInput.fill(this.testConfig.password);
        await page.waitForTimeout(500);
      }
      ...
      for (const selector of submitButtonSelectors) {
        try {
          ...
          if (submitButton) {
            console.log(`🚀 用户 ${userId} 找到提交按钮: ${selector}`);
            break;
          }
        } catch (e) {
          // 继续尝试下一个选择器
      ...
      if (submitButton) {
        ...
        console.log(`✅ 用户 ${userId} 点击了登录按钮`);
      } else {
        throw new Error('找不到登录提交按钮');
      }
      ...
      try {
        await Promise.race([
          ...
          page.waitForSelector('.el-message--success', { timeout: 8000 }),
          page.waitForSelector('[class*="success"]', { timeout: 8000 }),
        ...
        const successMessage = await page.$('.el-message--success') ||
                              ...
                              await page.$('.message-success');

        if (successMessage || currentUrl.includes('/dashboard') || currentUrl.includes('/home')) {
          result.success = true;
            ...
            const localStorage = await page.evaluate(() => {
              return {
                ...
                userInfo: localStorage.getItem('userInfo') ||
                           localStorage.getItem('kindergarten_user_info')
              };
            });
          ...
          console.log(`✅ 用户 ${userId} PC端管理员登录成功`);
        } else {
          const errorMessage = await page.$('.el-message--error') ||
                              await page.$('[class*="error"]') ||
                              await page.$('.message-error');

          if (errorMessage) {
            const errorText = await errorMessage.textContent();
            result.error = errorText || '登录失败';
          } else {
            result.error = '登录超时或未成功跳转';
          }
          console.log(`❌ 用户 ${userId} PC端管理员登录失败: ${result.error}`);
        }
      } catch (waitError) {
        result.error = '登录响应超时';
        console.log(`⏰ 用户 ${userId} PC端管理员登录超时`);
      }

    } catch (error) {
      result.error = error.message || String(error);
      console.log(`💥 用户 ${userId} PC端管理员登录异常: ${result.error}`);
    } finally {
  ...
  generateReport() {
    ...
    if (maxStableConcurrency < 5) {
      report.push('🔴 **紧急优化需求**: ');
      report.push('1. 立即检查前端页面渲染性能');
      report.push('2. 优化数据库查询和索引');
      report.push('3. 实施缓存策略');
      report.push('4. 考虑增加服务器资源');
      report.push('5. 检查登录页面组件是否存在问题');
    } else if (maxStableConcurrency < 15) {
      report.push('🟡 **中等优化需求**: ');
  ...
  async saveReport() {
    ...
    const reportPath = path.join(process.cwd(), `progressive-pc-login-test-report-${timestamp}.md`);

    await fs.writeFile(reportPath, report, 'utf8');
    console.log(`\n📊 测试报告已保存到: ${reportPath}`);

    // 保存原始数据
    const jsonReportPath = path.join(process.cwd(), `progressive-pc-login-test-data-${timestamp}.json`);
...
// 主执行函数
async function main() {
  const test = new ProgressivePCLoginTest();
  ...
  try {
    ...
    console.error('❌ 渐进式PC端压力测试失败:', error);
    process.exit(1);
  } finally {
    await test.cleanup();
  }
...
module.exports = ProgressivePCLoginTest;