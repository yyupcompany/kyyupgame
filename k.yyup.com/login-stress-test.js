...
class LoginStressTest {
  ...
  private concurrencyLevels = [10, 50, 100, 500];
  ...
  private async checkServices(): Promise<void> {
    ...
    // 检查后端API服务
    try {
      const apiResponse = await fetch(`${this.apiURL}/api/health`, {
        method: 'GET',
        timeout: 5000
      });
      if (!apiResponse.ok) {
        console.warn('⚠️ 后端健康检查失败，但继续测试');
      } else {
        console.log('✅ 后端API服务正常');
      }
    } catch (error) {
      console.warn('⚠️ 后端API服务不可用，但继续测试:', error);
    }
  }

  private async performLogin(config: TestConfig, userId: number): Promise<TestResult> {
  ...
  private async runConcurrencyTest(concurrency: number): Promise<ConcurrencyTestResult> {
